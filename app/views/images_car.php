    ...
    <div class="container">
        <div class="col-md-12">
            ...
            <OL CLASS="breadcrumb bg-white">
                ...
                <LI><a href="admin.php?page=images_car&id=<?php echo (isset($id_car)) ? $id_car : '' ; ?>"><?php echo (isset($id_car)) ? $id_car : '' ; ?></a></LI>
            </OL>
        </div>
    </div>
</SECTION>

<DIV><?php if(!empty($alert)) echo $alert ?></DIV>
...
<SECTION CLASS="row">
  <DIV CLASS="container">
    ...
    <DIV CLASS="box box-primary">  
      ...
      <DIV CLASS="box-body"> 
        <DIV ID="upload-cars" CLASS="upload-cars" DATA-URL="/app/modules/vehicles/upload_images_cars.php"></DIV>
        <DIV ID="status"></DIV>  
      </DIV>
    </DIV><!--//.box-->
  ...
  <form action="/app/modules/vehicles/delete_images_vehicles.php" id="form-delete-images-vehicles" name="form-delete-images-vehicles">
    ...
    <input type="hidden" name="categoria" value="images_cars">
    ...
    <DIV ID="reload-images">
      ...
      <?php 
        ...
        $images = $con->prepare("SELECT `id`, `id_car`, `image` FROM `images_cars` WHERE `id_car` = ? ORDER BY `id` DESC");
        $images->bind_param('i', $id_car);
        $images->execute();
        $images->store_result();
        $images->bind_result($id, $id_carro, $image);
        ...
        while( $reg = $images->fetch() )
        {
            ...
            $print_images .= '    </div>'; 
            $print_images .= '    <div class="select-capa define-capa-car" data-categoria="cars" data-img="'.$image.'" data-id-veiculo="'.$id_carro.'" TITLE="Clique para definir esta imagem como capa.">'; 
    ...
    <DIV CLASS="tfoot nav navbar-nav">    
      <button class="btn    btn-danger" type="submit" data-control="submit-del-images" DISABLED><I CLASS="fa fa-trash-o"></I> Excluir Ítens Selecionados</button>  
    </DIV> 
...
var settings = {
	url: url,
	method: "POST",
    formData: {"id-car":"<?php echo @$id_car ?>"},
