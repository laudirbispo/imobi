...
<SECTION CLASS="row">
    ...
    <div class="col-md-12">
        <a href="/app/admin/add_clients" class="btn btn-success btn-flat pull-right"><I CLASS="fa fa-user-plus"></I> <STRONG>Cadastrar Cliente</STRONG></a>
        ...
        <DIV CLASS="box box-solid">
          ...
          <DIV CLASS="box-body no-padding"> 
            ...
            if (!empty($_GET['select']) and isset($_GET['select']))
            {
                ...
                {
                    $where = "WHERE client_type = 'physical'";
                }
                else if ($select === 'person-juridical')
                {
                    $where = "WHERE client_type = 'juridical'";
                }
                else if ($select === 'owner')
                {
                    $where = "WHERE client_part = 'owner'";
                }
                else if ($select === 'tenant')
                {
                    $where = "WHERE client_part = 'tenant'";
                }
                else
                {
                    $where = '';
            ...
            $clients = $con->query("SELECT * FROM `clients` $where $order LIMIT $start,$limit");
            ...
            $item .= '<THEAD> ';
            $item .= '  <TR>';
            $item .= '    <TH CLASS="hidden-xs" STYLE="width:15% !important;">CLIENT_ID</TH>';
            $item .= '    <TH STYLE="width:40% !important;">CLIENTE</TH>';
            $item .= '    <TH STYLE="width:20% !important;">TIPO DE CLIENTE</TH> ';
            $item .= '    <TH STYLE="width:25% !important;">AÇÕES</TH> ';
            $item .= '  </TR> ';
            $item .= '</THEAD>';
            ...
            while($reg = $clients->fetch_array())
            {
              ...
              $item .= '<TD><a class="link" title="'.$client.'"><strong>'.substr_replace($client, (strlen($client) > 40 ? '...' : ''), 40).'</strong></a>';
              ...
              $item .= '</TD>';
              $item .= '<TD>'.$client_type.'</TD>';
              $item .= '<TD>';
              $item .= '<div class="btn-group">';
              $item .= '<button type="button" class="btn btn-default btn-sm">Ações</button>';
            ...
            // paginação----------
            //$busca_total = $con->query("SELECT COUNT(*) as `id` FROM `clients` $where");
