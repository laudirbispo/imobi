...
<SECTION CLASS="container-fluid">
    <div class="row">
        <div class="col-md-12">
            <H4 CLASS="text-darkgray"><STRONG>Imóveis Cadastrados</STRONG></H4>
...
<SECTION CLASS="container-fluid">
    ...
    <div class="col-md-12">
        <a href="/app/admin/add_properties" class="btn btn-success btn-flat pull-right"><I CLASS="fa fa-industry"></I> <STRONG>Cadastrar Imóvel</STRONG></a>
        <div class="clearfix space-20"></div>
    </div>
    
    <DIV CLASS="" id="table-properties">
       ...
       <?php
        ...
        if( $properties and $rows > 0)
        {
            ...
            while( $reg = $properties->fetch_assoc() )
            {
                ...
                if($reg['finality'] === 'venda-aluguel')
                {
                    ...
                    {
                        $venda = '<p>Venda:<strong class="text-blue"> R$ '.decimalMoeda( $reg['value_total']).' </strong> - <small>Sob consulta</small></p>';
                    }
                    else if($reg['hidden_value_total'] === 'Y' and empty( $reg['value_total']))
                    {
                        $venda = '<p>Venda:<strong class="text-blue"> Não informado </strong> - <small>Sob consulta</small></p>';
                    }
                    else if($reg['hidden_value_total'] === 'N' and !empty( $reg['value_total']))
                    {
                        $venda = '<p>Venda:<strong class="text-blue"> R$ '.decimalMoeda( $reg['value_total']).' </strong></p>';
                    }
                    else if($reg['hidden_value_total'] === 'N' and empty( $reg['value_total']))
                    {
                    ...
                    {
                        $aluguel = '<p>Aluguel:<strong class="text-blue"> R$ '.decimalMoeda($reg['value_monthly']).' </strong> - <small>Sob consulta</small></p>';
                    }
                    else if($reg['hidden_value_monthly'] === 'Y' and empty($reg['value_monthly']))
                    {
                        $aluguel = '<p>Aluguel:<strong class="text-blue"> Não informado </strong> - <small>Sob consulta</small></p>';
                    }
                    else if($reg['hidden_value_monthly'] === 'N' and !empty($reg['value_monthly']))
                    {
                        $aluguel = '<p>Aluguel:<strong class="text-blue"> R$ '.decimalMoeda($reg['value_monthly']).' </strong></p>';
                    }
                    else if($reg['hidden_value_monthly'] === 'N' and empty($reg['value_monthly']))
                    {
                ...
                else if( $reg['finality'] === 'aluguel' or $reg['finality'] === 'temporada' or $reg['finality'] === 'arrendamento')
                {
                    ...
                    if($reg['hidden_value_monthly'] == 'Y' and !empty($reg['value_monthly']))
                    {
                        $aluguel = '<p>Aluguel:<strong class="text-blue"> R$ '.decimalMoeda($reg['value_monthly']).' </strong> - Sob consulta</p>';
                    }
                    else if($reg['hidden_value_monthly'] == 'Y' and empty($reg['value_monthly']))
                    {
                        $aluguel = '<p>Aluguel:<strong class="text-blue"> Não informado </strong> - <small>Sob consulta</small></p>';
                ...
                if($reg['featured'] == 'Y')
                {
                    $featured = '<a class="featured text-yellow" data-action="unfeatured" href="javascript:;" title="Remover dos destaques" data-tid="'.$reg['id'].'"><i class="fa fa-2x fa-star"></i></a>';
                }
                else if($reg['featured'] == 'N')
                {
                    $featured = '<a class="featured text-yellow" data-action="featured" href="javascript:;" title="Marcar como destaque" data-tid="'.$reg['id'].'"><i class="fa fa-2x fa-star-o"></i></a>';
                }
                else
                {
                    $featured = '';
                ...
                $item .= '<div class="col-md-3 col-sm-4 col-xs-12" data-control="elem-filter">';
                $item .= '<div class="card flat animated zoomIn">';
                $item .= $status;
                $item .= $featured;
                ...
                $item .= '<h5 class="text-capitalize">'.$reg['type'].' para '.$finality.'</h5>';
                $item .= '<small class="label label-info flat"><strong>REF:</strong> '.$reg['ref'].'</small>';
                $item .= '</div>';
                $item .= '</div>';
                $item .= '<div class="card-content card-properties">';
                $item .= '<small class="text-blue text-uppercase">'.$reg['segment'].'</small>';
                $item .= '<p class="text-darkgray">'.$reg['address_street'].', '.$reg['address_neighborhood'].'  '.$reg['address_number'].', '.$reg['address_city'].'-'.$reg['address_state'].'</p>';
                $item .= $venda;
                $item .= $aluguel;
                $item .= '</div>';
                ...
                $item .= '<li class="divider"></li>';
                $item .= '<li><a HREF="javascript:;" role="BUTTON" data-control="del-properties" data-tid="'.$reg['id'].'"><I CLASS="fa fa-trash text-red"></I> Deletar imóvel</a></li>';
                $item .= '</ul>';
                $item .= '</div>';
                $item .= '</div>';
                $item .= '</div>';
        ...
        <div class="clearfix col-md-12">
            <p class="text-info"><strong><?php echo $total ?> imóveis encontrados </strong></p>
        </div>
