...
$insert_car = $con->prepare(" INSERT INTO `cars` (`marca`, `modelo`, `submodelo`, `ano_fab`, `ano_mod`, `combustivel`, `valor`, `carroceria`, `km`, `cor`, `portas`, `final_placa`, `cambio`, `obs`, `autor_post`, `date_post`, `local_loja`, `unico_dono`, `troca`, `estado`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ");
$insert_car->bind_param('sssiisssssssssissss', $car_marca, $car_modelo, $car_submodelo, $car_ano_fab, $car_ano_mod, $car_combustivel, $car_valor, $car_carroceria, $car_km, $car_cor, $car_portas, $car_placa, $car_cambio, $car_obs, $user_id, $date_time, $car_loja, $car_unico_dono, $car_troca, $car_estado)
;
$insert_car->execute();
$insert_car_rows = $insert_car->affected_rows;
$last_id = $insert_car->insert_id;
$insert_car->close();

if( $insert_car and $insert_car_rows > 0 ) 
{
   $opcionais = $con->prepare(" INSERT INTO `cars_optional` (`id_car`, `airbag`, `ar_condicionado`, `direcao`, `alarme`, `cd_player`, `rodas`, `travas`, `vidros`, `camera`, `sensor_estacionamento`, `chave_reserva`, `manual`, `desembacador_traseiro`, `computador`, `ar_quente`, `multimidia`, `controles_volante`, `volante_regulagem`, `retrovisor`, `gps`, `abs`, `farois_neblina`, `capota`, `teto_solar`, `bancos_couro`, `farois_regulagem`, `sensor_chuva`, `brake_light`, `limpador_traseiro`, `aerofolio`, `engate_traseiro`, `estribo`, `santantonio`, `mascara_negra`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ");
   $opcionais->bind_param('iiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii', $last_id, $car_airbag, $car_ar_condicionado, $car_direcao, $car_alarme, $car_cd_player, $car_rodas, $car_travas, $car_vidros, $car_camera, $car_estacionamento, $car_chave_reserva, $car_manual, $car_desembacador, $car_computador, $car_ar_quente, $car_multimidia, $car_controles_volante, $car_volante_regulagem, $car_retrovisor, $car_gps, $car_abs, $car_neblina, $car_capota, $car_teto_solar, $car_couro, $car_farois_regulagem, $car_sensor_chuva, $car_brake_light, $car_limpador_traseiro, $car_aerofolio, $car_engate, $car_estribo, $car_santoantonio, $car_mascara_negra);
   $opcionais->execute();
   $rows_opcionais = $opcionais->affected_rows;
   $opcionais->close();
    
    if( $opcionais and $rows_opcionais > 0)
    {
        $dir = $_SERVER['DOCUMENT_ROOT'].'/docs/vehicles/cars/'.$last_id;
        ...
        $response = array(
            ...
            'link'    => 'admin.php?page=images_car&id='.base64_encode($last_id),
          );
        die(json_encode($response));
    }
    else
    {
       $del = $con->query("DELETE FROM `cars` WHERE `cars`.`id` = '$last_id' ");
       $response = array(
            'status'  => 'error',
...
{ 
    $response = array(
            'status'  => 'error',
            'message' => 'Falha ao cadastrar veículo '.$last_id,
            'link'    => '',
          );
    die(json_encode($response));
}
